...
public class TrolCommander {
	...
	/**
	 * true while the application is launching, false after it has finished
	 * launching
	 */
	...
	/**
	 * This method can be called to wait until the application has been
	 * launched. The caller thread will be blocked until the application has
	 * been launched. This method will return immediately if the application has
	 * already been launched when it is called.
	 */
	...
	// --------------------------------------------------------------------------
	/**
	 * Method used to migrate commands that used to be defined in the
	 * configuration but were moved to <code>commands.xml</code>.
	 * 
	 * @param useName
	 *            name of the <code>use custom command</code> configuration
	 *            variable.
	 * @param commandName
	 *            name of the <code>custom command</code> configuration
	 *            variable.
	 */
	...
	/**
	 * Checks whether a graphics environment is available and exit with an error
	 * otherwise.
	 */
	...
	private static class LoadConfigsTask extends LauncherTask {
		...
		void run() throws Exception {
			// Attempts to guess whether this is the first time muCommander is
			// booted or not.
			// boolean isFirstBoot;
			// try {isFirstBoot = !MuConfigurations.isPreferencesFileExists();}
			// catch(IOException e) {isFirstBoot = true;}

			// Load snapshot data before loading configuration as until version
			// 0.9 the snapshot properties
			// were stored as preferences so when loading such preferences they
			// could overload snapshot properties
			try {
			...
			// is performed : under Mac OS X, if we're
			...
			// The math.max(1.0f, ...) part is to workaround a bug which
			// cause(d) this value to be set to 0.0 in the configuration file.
			FileIcons.setScaleFactor(Math.max(1.0f, MuConfigurations.getPreferences()
					.getVariable(MuPreference.TABLE_ICON_SCALE, MuPreferences.DEFAULT_TABLE_ICON_SCALE)));
			FileIcons.setSystemIconsPolicy(MuConfigurations.getPreferences()
	...
	private static class LoadIconsTask extends LauncherTask {
		...
		void run() throws Exception {
			...
			// Initialize the SwingFileIconProvider from the main thread, see
			// method Javadoc for an explanation on why we do this now
	...
	private static class EnableNotificationsTask extends LauncherTask {
		...
		@Override
		void run() throws Exception {
			// Enable system notifications, only after MainFrame is created as
			// SystemTrayNotifier needs to retrieve
			// a MainFrame instance
			if (MuConfigurations.getPreferences().getVariable(MuPreference.ENABLE_SYSTEM_NOTIFICATIONS,
	...
	private static class StartBonjourTask extends LauncherTask {
		...
		void run() throws Exception {
			...
			com.mucommander.bonjour.BonjourDirectory.setActive(MuConfigurations.getPreferences().getVariable(
					MuPreference.ENABLE_BONJOUR_DISCOVERY, MuPreferences.DEFAULT_ENABLE_BONJOUR_DISCOVERY));
		}
	}
	...
	private static class StartTask extends LauncherTask {
		...
		void run() throws Exception {
			...
			// ------------------------------------------------------------
			// If trolCommander is running under Mac OS X (how lucky!), add some
			// glue for the main menu bar and other OS X
			// specifics.
			if (OsFamily.MAC_OS_X.isCurrent()) {
				// Use reflection to create an OSXIntegration instance so that
				// ClassLoader
				// doesn't throw an NoClassDefFoundException under platforms
				// other than Mac OS X
				try {
			...
			// This the property is supposed to have the java.net package use
			// the proxy defined in the system settings
			// to establish HTTP connections. This property is supported only
			// under Java 1.5 and up.
			// Note that Mac OS X already uses the system HTTP proxy, with or
			// without this property being set.
			System.setProperty("java.net.useSystemProxies", "true");

			// boolean showSetup =
			// MuConfigurations.getPreferences().getVariable(MuPreference.THEME_TYPE)
			// == null;
			// Traps VM shutdown
	...
	private static class ShowSplashTask extends LauncherTask {
		...
		void run() throws Exception {
			...
			useSplash = MuConfigurations.getPreferences().getVariable(MuPreference.SHOW_SPLASH_SCREEN,
	...
	private static class ConfigureFsTask extends LauncherTask {
		...
		@Override
		void run() throws Exception {
			// Configure the SMB subsystem (backed by jCIFS) to maintain
			// compatibility with SMB servers that don't support
			// NTLM v2 authentication such as Samba 3.0.x, which still is widely
			// used and comes pre-installed on
			// Mac OS X Leopard.
			// Since jCIFS 1.3.0, the default is to use NTLM v2 authentication
			// and extended security.
			SMBProtocolProvider.setLmCompatibility(MuConfigurations.getPreferences()
					.getVariable(MuPreference.SMB_LM_COMPATIBILITY, MuPreferences.DEFAULT_SMB_LM_COMPATIBILITY));
			SMBProtocolProvider.setExtendedSecurity(MuConfigurations.getPreferences().getVariable(
					MuPreference.SMB_USE_EXTENDED_SECURITY, MuPreferences.DEFAULT_SMB_USE_EXTENDED_SECURITY));

			// Use the FTP configuration option that controls whether to force
			// the display of hidden files, or leave it for
			// the servers to decide whether to show them.
			FTPProtocolProvider.setForceHiddenFilesListing(MuConfigurations.getPreferences()
			...
			FileFactory.registerProtocol(BookmarkProtocolProvider.BOOKMARK,
	...
	private static class InitCustomDateFormatTask extends LauncherTask {
		...
		@Override
		void run() throws Exception {
			// Inits CustomDateFormat to make sure that its
			// ConfigurationListener is added
			// before FileTable, so CustomDateFormat gets notified of date
	...
	private static class LauncherExecutor extends ThreadPoolExecutor {
		...
		public boolean execute(LauncherTask task, boolean force) {
			if (force || (runningTasks.size() < cores && task.isReadyForExecution())) {
				...
				// task.depends.length + " " + task.isReadyForExecution() + " "
				// + runningTasks);
	...
	public static void main(String args[]) throws IOException {
		...
		try {
			...
			LauncherTask taskRegisterActions = new LoadActionsTask(helper); // new
			...
			LauncherTask taskCreateWindow = new CreateWindowTask(helper, taskLoadTheme, taskShowSplash, taskInitBars,
					taskRegisterActions);
		...
		executor.shutdown();
		/*
		 * try { executor.awaitTermination(100, TimeUnit.SECONDS); } catch
		 * (InterruptedException e) { e.printStackTrace(); }
		 */
		// Done launching, wake up threads waiting for the application being
		// launched.
		// Important: this must be done before disposing the splash screen, as
		// this would otherwise create a deadlock
		...
		if (MuConfigurations.getPreferences().getVariable(MuPreference.CHECK_FOR_UPDATE,
			...
			SwingUtilities.invokeLater(() -> {
				try {
					...
					String message = RuntimeConstants.VERSION_URL
							+ " cannot be accessed. You're offline or the server is down for now. " + e.getMessage()
							+ " Enable trace to get the full expected error.";
					if (match(e, java.net.ConnectException.class, "Connection refused: connect")
							|| match(e, java.net.UnknownHostException.class, "trolsoft.ru")
							|| match(e, IOException.class, "Not Found")) {
						LOGGER.info(message);
	...
	private static <T1 extends Throwable, T2 extends T1> boolean match(T1 exception, Class<T2> clazz, String message) {
		return clazz.isInstance(exception) && exception.getMessage().equals(message);
	}
}
